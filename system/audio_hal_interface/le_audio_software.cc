...
// Build a list of LeAudioCodecIds that MM (offload) actually supports, derived
// from the codec preference list MM passed to CodecManager::Start().
...
BuildMmSupportedCodecIds() {
  ...
  for (const auto& cfg : cm->GetOffloadingPreference()) {
    ...
    if (cfg.codec_type == ::bluetooth::le_audio::LE_AUDIO_CODEC_INDEX_SOURCE_VENDOR_SPECIFIC &&
        cfg.codec_id != 0) {
      // MM sends vendor codecs with codec_type=VENDOR_SPECIFIC and the full
      // 40-bit ASCS codec ID in codec_id. Use that to recover the exact
      // LeAudioCodecId (coding_format, vendor_company_id, vendor_codec_id).
      id = ::bluetooth::le_audio::utils::translateCodecIdToLeAudioCodecId(cfg.codec_id);
    } else {
      id = translateCodecTypeToLeAudioCodecId(cfg.codec_type);
    }
...
void LeAudioClientInterface::Sink::CancelStreamingRequestWithUnsupported() {
  ...
  if (instance->IsRequestCompletedAfterUpdate(lambda, BluetoothRequest::RESUME)) {
    interface->StreamStarted(aidl::BluetoothAudioCtrlAck::FAILURE_UNSUPPORTED);
  }
}
...
void LeAudioClientInterface::Sink::UpdateMetadataChanged(::bluetooth::le_audio::types::AseState&
  ...
  get_aidl_sink_client_interface(is_broadcaster_)->onSinkAseMetadataChanged(ase_state, cig_id, cis_id, aidl_metadata);
}
...
LeAudioClientInterface::Sink::GetVendorConfigureDataPathPayload(
  ...
  auto aidl_config_payload = get_aidl_sink_client_interface(is_broadcaster_)
...
void LeAudioClientInterface::Source::UpdateMetadataChanged(::bluetooth::le_audio::types::AseState&
  ...
  aidl::le_audio::LeAudioSourceTransport::interface_unicast_->onSourceAseMetadataChanged(ase_state, cig_id, cis_id, aidl_metadata);
}
